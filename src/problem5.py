...
def problem5(seq_of_seq):
    ...
    for k in range(len(seq_of_seq)):
        inner_seq = seq_of_seq[k]
        if len(inner_seq) > 0:
            largest_value = inner_seq[0]
            for j in range(len(inner_seq)):
                if inner_seq[j] > largest_value:
                    largest_value = inner_seq[j]

            lis = lis + [largest_value]
